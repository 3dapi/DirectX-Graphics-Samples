...
void D3D12MeshletCull::LoadAssets()
{
    // Load shaders, root signature, and create the the pipeline state.
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateRootSignature(0, ampShader.data, ampShader.size, IID_PPV_ARGS(&m_d3dRootSignature)));
    ...
    for (uint32_t i = 0; i < static_cast<uint32_t>(m_objects.size()); ++i)
    {
        ...
        obj.Model.UploadGpuResources(m_d3dDevice.Get(), m_d3dCommandQueue.Get(), m_commandAllocators[m_d3dCurrentFrameIndex].Get(), m_commandList.Get());
    ...
    m_frustumDraw.CreateDeviceResources(m_d3dDevice.Get(), m_d3dRenderTarget[0]->GetDesc().Format, m_d3dDepthStencil->GetDesc().Format);
    m_cullDataDraw.CreateDeviceResources(m_d3dDevice.Get(), m_d3dRenderTarget[0]->GetDesc().Format, m_d3dDepthStencil->GetDesc().Format);
}
...
void D3D12MeshletCull::OnUpdate()
{
...

    {
        ...
        auto& constants = *(reinterpret_cast<Constants*>(m_constantsData) + m_d3dCurrentFrameIndex);
    ...
    for (auto& obj : m_objects)
    {
        ...
        auto& instance = *(reinterpret_cast<Instance*>(obj.InstanceData) + m_d3dCurrentFrameIndex);
...
void D3D12MeshletCull::PopulateCommandList()
{
    ...
    {
        // Bind instance data
        m_commandList->SetGraphicsRootConstantBufferView(2, obj.InstanceResource->GetGPUVirtualAddress() + sizeof(Instance) * m_d3dCurrentFrameIndex);
