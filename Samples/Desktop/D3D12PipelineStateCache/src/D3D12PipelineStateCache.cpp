...
void D3D12PipelineStateCache::LoadAssets()
{
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[m_d3dCurrentFrameIndex].Get(), nullptr, IID_PPV_ARGS(&m_commandList)));
...
void D3D12PipelineStateCache::PopulateCommandList()
{
    ...
    // re-recording.
    ThrowIfFailed(m_commandList->Reset(m_commandAllocators[m_d3dCurrentFrameIndex].Get(), nullptr));
    ...
    PIXBeginEvent(m_commandList.Get(), 0, L"Draw cube");
    {
        ...
        DrawConstantBuffer* drawCB = (DrawConstantBuffer*)m_dynamicCB.GetMappedMemory(m_drawIndex, m_d3dCurrentFrameIndex);
        ...
        m_psoLibrary.SetPipelineState(m_d3dDevice.Get(), m_d3dRootSignature.Get(), m_commandList.Get(), BaseNormal3DRender, m_d3dCurrentFrameIndex);

        m_commandList->SetGraphicsRootConstantBufferView(RootParameterCB, m_dynamicCB.GetGpuVirtualAddress(m_drawIndex, m_d3dCurrentFrameIndex));
    ...
    PIXBeginEvent(m_commandList.Get(), 0, L"Post-processing");
    {
        ...
        for (UINT i = PostBlit; i < EffectPipelineTypeCount; i++)
        {
            if (m_enabledEffects[i])
            {
                ...
                m_psoLibrary.SetPipelineState(m_d3dDevice.Get(), m_d3dRootSignature.Get(), m_commandList.Get(), static_cast<EffectPipelineType>(i), m_d3dCurrentFrameIndex);
