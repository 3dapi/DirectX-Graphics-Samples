...
void D3D12HelloFrameBuffering::LoadAssets()
{
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[m_d3dCurrentFrameIndex].Get(), m_pipelineState.Get(), IID_PPV_ARGS(&m_d3dCommandList)));
...
void D3D12HelloFrameBuffering::PopulateCommandList()
{
    ...
    // re-recording.
    ThrowIfFailed(m_d3dCommandList->Reset(m_commandAllocators[m_d3dCurrentFrameIndex].Get(), m_pipelineState.Get()));
