...
void D3D12Residency::LoadAssets()
{
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_d3dCommandAllocator.Get(), m_pipelineState.Get(), IID_PPV_ARGS(&m_commandList)));
...
void D3D12Residency::LoadTexturesAsync()
{
    ...
    const auto& CreateAndUpload = [this]()
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateCommandList(0, queueDesc.Type, commandAllocator.Get(), m_pipelineState.Get(), IID_PPV_ARGS(&commandList)));
...
void D3D12Residency::OnRender()
{
    ...
    if (m_commandListPool.empty() || m_commandListPool.front()->fenceValue > m_fence->GetCompletedValue())
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, pManagedCommandList->commandAllocator.Get(), m_pipelineState.Get(), IID_PPV_ARGS(&pManagedCommandList->commandList)));
...
void D3D12Residency::PopulateCommandList(std::shared_ptr<ManagedCommandList> pManagedCommandList)
{
    ...
    commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_d3dRenderTarget[m_d3dCurrentFrameIndex].Get(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET));
    ...
    commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_d3dRenderTarget[m_d3dCurrentFrameIndex].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT));
