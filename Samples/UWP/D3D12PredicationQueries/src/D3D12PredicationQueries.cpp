...
void D3D12PredicationQueries::LoadPipeline()
{
    ...
    // Create descriptor heaps.
    {
        ...
        cbvHeapDesc.NumDescriptors = CbvCountPerFrame * m_RenderTargetCount;
        ...
        ThrowIfFailed(m_d3dDevice->CreateQueryHeap(&queryHeapDesc, IID_PPV_ARGS(&m_queryHeap)));
...
void D3D12PredicationQueries::LoadAssets()
{
    ...
    // Create the pipeline state, which includes compiling and loading shaders.
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_queryState)));
    ...
    // Create the constant buffers.
    {
        ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
            &CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
            D3D12_HEAP_FLAG_NONE,
            &CD3DX12_RESOURCE_DESC::Buffer(m_RenderTargetCount * sizeof(m_constantBufferData)),
            D3D12_RESOURCE_STATE_GENERIC_READ,
            nullptr,
        ...
        ZeroMemory(m_pCbvDataBegin, m_RenderTargetCount * sizeof(m_constantBufferData));
...
void D3D12PredicationQueries::PopulateCommandList()
{
    ...
    // Draw the quads and perform the occlusion query.
    {
        ...
        PIXBeginEvent(m_d3dCommandList.Get(), 0, L"Draw potentially occluded geometry");
        m_d3dCommandList->SetGraphicsRootDescriptorTable(0, cbvFarQuad);
        m_d3dCommandList->SetPredication(m_queryResult.Get(), 0, D3D12_PREDICATION_OP_EQUAL_ZERO);
        m_d3dCommandList->DrawInstanced(4, 1, 0, 0);
        PIXEndEvent(m_d3dCommandList.Get());

        // Disable predication and always draw the near quad.
        PIXBeginEvent(m_d3dCommandList.Get(), 0, L"Draw animating geometry");
        m_d3dCommandList->SetPredication(nullptr, 0, D3D12_PREDICATION_OP_EQUAL_ZERO);
        m_d3dCommandList->SetGraphicsRootDescriptorTable(0, cbvNearQuad);
        m_d3dCommandList->DrawInstanced(4, 1, 4, 0);
        PIXEndEvent(m_d3dCommandList.Get());

        // Run the occlusion query with the bounding box quad.
        PIXBeginEvent(m_d3dCommandList.Get(), 0, L"Execute occlusion query");
        m_d3dCommandList->SetGraphicsRootDescriptorTable(0, cbvFarQuad);
        m_d3dCommandList->SetPipelineState(m_queryState.Get());
        m_d3dCommandList->BeginQuery(m_queryHeap.Get(), D3D12_QUERY_TYPE_BINARY_OCCLUSION, 0);
        m_d3dCommandList->DrawInstanced(4, 1, 8, 0);
        m_d3dCommandList->EndQuery(m_queryHeap.Get(), D3D12_QUERY_TYPE_BINARY_OCCLUSION, 0);
        PIXEndEvent(m_d3dCommandList.Get());

        // Resolve the occlusion query and store the results in the query result buffer
        // to be used on the subsequent frame.
        m_d3dCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_queryResult.Get(), D3D12_RESOURCE_STATE_PREDICATION, D3D12_RESOURCE_STATE_COPY_DEST));
        m_d3dCommandList->ResolveQueryData(m_queryHeap.Get(), D3D12_QUERY_TYPE_BINARY_OCCLUSION, 0, 1, m_queryResult.Get(), 0);
        m_d3dCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_queryResult.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PREDICATION));
    }
