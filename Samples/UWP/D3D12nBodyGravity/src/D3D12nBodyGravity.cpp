...
void D3D12nBodyGravity::LoadPipeline()
{
    ...
    m_d3dSwapChainEvent = m_d3dSwapChain->GetFrameLatencyWaitableObject();
    ...
    // Create descriptor heaps.
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&srvUavHeapDesc, IID_PPV_ARGS(&m_srvUavHeap)));
        ...
        m_srvUavDescriptorSize = m_d3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
...
void D3D12nBodyGravity::LoadAssets()
{
    // Create the root signatures.
    {
        ...
        // Compute root signature.
        {
            ...
            ThrowIfFailed(m_d3dDevice->CreateRootSignature(0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&m_computeRootSignature)));
    ...
    // Create the pipeline states, which includes compiling and loading shaders.
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateComputePipelineState(&computePsoDesc, IID_PPV_ARGS(&m_computeState)));
        NAME_D3D12_OBJECT(m_computeState);
    }

    // Create the command list.
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_d3dCommandAllocators[m_frameIndex].Get(), m_d3dPipelineState.Get(), IID_PPV_ARGS(&m_d3dCommandList)));
    ...
    // Create the compute shader's constant buffer.
    {
        ...
        UpdateSubresources<1>(m_d3dCommandList.Get(), m_constantBufferCS.Get(), constantBufferCSUpload.Get(), 0, 0, 1, &computeCBData);
        m_d3dCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_constantBufferCS.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER));
    }

    // Create the geometry shader's constant buffer.
    {
        const UINT constantBufferGSSize = sizeof(ConstantBufferGS) * m_RenderTargetCount;
    ...
    {
        ThrowIfFailed(m_d3dDevice->CreateFence(m_renderContextFenceValue, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_renderContextFence)));
...
void D3D12nBodyGravity::CreateVertexBuffer()
{
    ...
    UpdateSubresources<1>(m_d3dCommandList.Get(), m_vertexBuffer.Get(), m_vertexBufferUpload.Get(), 0, 0, 1, &vertexData);
...
void D3D12nBodyGravity::CreateParticleBuffers()
{
    ...
    for (UINT index = 0; index < ThreadCount; index++)
    {
        ...
        UpdateSubresources<1>(m_d3dCommandList.Get(), m_particleBuffer0[index].Get(), m_particleBuffer0Upload[index].Get(), 0, 0, 1, &particleData);
        UpdateSubresources<1>(m_d3dCommandList.Get(), m_particleBuffer1[index].Get(), m_particleBuffer1Upload[index].Get(), 0, 0, 1, &particleData);
        m_d3dCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_particleBuffer0[index].Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
        m_d3dCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_particleBuffer1[index].Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE));
        ...
        m_d3dDevice->CreateShaderResourceView(m_particleBuffer0[index].Get(), &srvDesc, srvHandle0);
        m_d3dDevice->CreateShaderResourceView(m_particleBuffer1[index].Get(), &srvDesc, srvHandle1);
        ...
        m_d3dDevice->CreateUnorderedAccessView(m_particleBuffer0[index].Get(), nullptr, &uavDesc, uavHandle0);
        m_d3dDevice->CreateUnorderedAccessView(m_particleBuffer1[index].Get(), nullptr, &uavDesc, uavHandle1);
    }
}
...
void D3D12nBodyGravity::CreateAsyncContexts()
{
    for (UINT threadIndex = 0; threadIndex < ThreadCount; ++threadIndex)
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&m_computeCommandQueue[threadIndex])));
        ThrowIfFailed(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_COMPUTE, IID_PPV_ARGS(&m_computeAllocator[threadIndex])));
        ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE, m_computeAllocator[threadIndex].Get(), nullptr, IID_PPV_ARGS(&m_computeCommandList[threadIndex])));
        ThrowIfFailed(m_d3dDevice->CreateFence(0, D3D12_FENCE_FLAG_SHARED, IID_PPV_ARGS(&m_threadFences[threadIndex])));
...
void D3D12nBodyGravity::OnUpdate()
{
    ...
    WaitForSingleObjectEx(m_d3dSwapChainEvent, 100, FALSE);
    ...
    try
    {
        ...
        for (UINT n = 0; n < ThreadCount; n++)
        {
            ...
            if (m_threadFences[n]->GetCompletedValue() < threadFenceValue)
            {
                ...
                ThrowIfFailed(m_d3dCommandQueue->Wait(m_threadFences[n].Get(), threadFenceValue));
            }
        }
...
void D3D12nBodyGravity::WaitForGpu()
{
    ...
    ThrowIfFailed(m_d3dCommandQueue->Signal(m_renderContextFence.Get(), m_renderContextFenceValues[m_frameIndex]));
...
void D3D12nBodyGravity::PopulateCommandList()
{
    ...
    // re-recording.
    ThrowIfFailed(m_d3dCommandList->Reset(m_d3dCommandAllocators[m_frameIndex].Get(), m_d3dPipelineState.Get()));

    // Set necessary state.
    m_d3dCommandList->SetPipelineState(m_d3dPipelineState.Get());
    m_d3dCommandList->SetGraphicsRootSignature(m_d3dRootSignature.Get());

    m_d3dCommandList->SetGraphicsRootConstantBufferView(GraphicsRootCBV, m_constantBufferGS->GetGPUVirtualAddress() + m_frameIndex * sizeof(ConstantBufferGS));

    ID3D12DescriptorHeap* ppHeaps[] = { m_srvUavHeap.Get() };
    m_d3dCommandList->SetDescriptorHeaps(_countof(ppHeaps), ppHeaps);

    m_d3dCommandList->IASetVertexBuffers(0, 1, &m_vertexBufferView);
    m_d3dCommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_POINTLIST);
    ...
    for (UINT n = 0; n < ThreadCount; n++)
    {
        ...
        m_d3dCommandList->SetGraphicsRootDescriptorTable(GraphicsRootSRVTable, srvHandle);

        PIXBeginEvent(m_d3dCommandList.Get(), 0, L"Draw particles for thread %u", n);
        m_d3dCommandList->DrawInstanced(ParticleCount, 1, 0, 0);
...
void D3D12nBodyGravity::WaitForRenderContext()
{
    ...
    ThrowIfFailed(m_d3dCommandQueue->Signal(m_renderContextFence.Get(), m_renderContextFenceValue));
