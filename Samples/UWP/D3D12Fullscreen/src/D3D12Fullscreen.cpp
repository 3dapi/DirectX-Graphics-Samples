...
void D3D12Fullscreen::LoadPipeline()
{
    ...
    // Create descriptor heaps.
    {
        ...
        rtvHeapDesc.NumDescriptors = m_RenderTargetCount + 1; // + 1 for the intermediate render target.
        ...
        cbvSrvHeapDesc.NumDescriptors = m_RenderTargetCount + 1; // One CBV per frame and one SRV for the intermediate render target.
...
void D3D12Fullscreen::LoadAssets()
{
    ...
    {
        ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_sceneCommandAllocators[m_frameIndex].Get(), m_scenePipelineState.Get(), IID_PPV_ARGS(&m_sceneCommandList)));
        NAME_D3D12_OBJECT(m_sceneCommandList);

        ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_postCommandAllocators[m_frameIndex].Get(), m_postPipelineState.Get(), IID_PPV_ARGS(&m_postCommandList)));
    ...
    // Create the constant buffer.
    {
        ThrowIfFailed(m_d3dDevice->CreateCommittedResource(
            &CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
            D3D12_HEAP_FLAG_NONE,
            &CD3DX12_RESOURCE_DESC::Buffer(sizeof(SceneConstantBuffer) * m_RenderTargetCount),
            D3D12_RESOURCE_STATE_GENERIC_READ,
            nullptr,
...
void D3D12Fullscreen::OnSizeChanged(UINT width, UINT height, bool minimized)
{
    ...
    if ((width != m_width || height != m_height) && !minimized)
    {
        ...
        ThrowIfFailed(m_d3dSwapChain->ResizeBuffers(m_RenderTargetCount, width, height, desc.BufferDesc.Format, desc.Flags));
...
void D3D12Fullscreen::PopulateCommandLists()
{
    ...
    // Populate m_sceneCommandList to render scene to intermediate render target.
    {
        ...
        CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(m_d3dDecsHeap->GetCPUDescriptorHandleForHeapStart(), m_RenderTargetCount, m_d3dDescriptorSize);
...
void D3D12Fullscreen::LoadSceneResolutionDependentResources()
{
    ...
    {
        D3D12_RESOURCE_DESC swapChainDesc = m_d3dRenderTarget[m_frameIndex]->GetDesc();
